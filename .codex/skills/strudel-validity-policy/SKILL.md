...
6. For jam output, obey JSON shape exactly: `{"pattern":"...","thoughts":"...","reaction":"..."}`.
...
Target: per-agent JSON response with a valid single pattern.

```json
{
  "pattern": "s(\"bd [~ bd] sd [bd ~]\").bank(\"RolandTR909\").gain(0.9).sometimes(x => x.fast(2))",
